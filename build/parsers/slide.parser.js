...
;
export const parseSlide = async (slideId, presentation) => {
    const slide = {
        theme: null,
    };
    ...
    const slideLayout = await parseSlideComponent(slideDocument.relations.find(relation => relation.type.includes("relationships/slideLayout")).url, { root: "p:sldLayout", relations: slideDocument.relations });
    const slideMaster = await parseSlideComponent(slideLayout.relations.find(relation => relation.type.includes("relationships/slideMaster")).url, { root: "p:sldMaster", relations: slideLayout.relations });
...
const parseSlideComponent = async (path, options = {}) => {
    const document = await readSlideFile(path);
    const slide = {
        properties: {},
        colorMap: null,
        textStyles: null,
    ...
    if (documentRoot["p:cSld"][0]) {
        ...
        const documentShapes = await resolveRelations(parseSlideShapes(slideContent["p:spTree"][0], options), document.relations);
...
const resolveRelations = (shapes, relations) => {
    return Promise.all(shapes.filter(shape => shape).map(async (shape) => {
