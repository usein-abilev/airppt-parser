...
export const getThemeColor = (color, theme?: any) => {
    if (!color) return { type: FillType.NO_FILL, opacity: 1, };
...
const parseFillStyleList = (fillStyleList) => {
    if (!fillStyleList) return [];
    const list = Object.keys(fillStyleList).map(item => {
        const list = fillStyleList[item];
        if (item === "a:gradFill") return parseFillsByType(list, parseGradientFill);
        else if (item === "a:solidFill") return parseFillsByType(list, parseSolidFill);
        else if (item === "a:pattFill") return parseFillsByType(list, parsePatternFill);
        else {
